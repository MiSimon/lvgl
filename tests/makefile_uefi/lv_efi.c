...
lv_display_t* lv_efi_init_display(EFI_GRAPHICS_OUTPUT_PROTOCOL* protocol)
{
    ...
    private_data->render_buffer_size = sizeof(EFI_GRAPHICS_OUTPUT_BLT_PIXEL) * protocol->Mode->Info->HorizontalResolution * protocol->Mode->Info->VerticalResolution;
    ...
    lv_display_set_buffers(display, private_data->render_buffer, NULL, private_data->render_buffer_size, LV_DISPLAY_RENDER_MODE_DIRECT);
...
static void _s_indev_kb_read_cb(lv_indev_t *indev, lv_indev_data_t *data)
{
    ...
    else
    {
        ...
        if (status != EFI_SUCCESS)
        {
            ...
            {
                LV_LOG_ERROR("ReadKeyStroke failed.");
            }
            return;
        }
