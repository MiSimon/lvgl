...
static void lv_uefi_simple_pointer_indev_event_cb(lv_event_t * e);
static void lv_uefi_simple_pointer_read_cb(lv_indev_t * indev, lv_indev_data_t * data);
static void lv_uefi_simple_pointer_handle_context_free(void * ptr);
static void lv_uefi_simple_pointer_context_free(lv_uefi_simple_pointer_context_t * indev_ctx);
static bool lv_uefi_simple_pointer_interface_is_valid(const EFI_SIMPLE_POINTER_PROTOCOL * interface);
static void lv_uefi_simple_pointer_read(lv_uefi_simple_pointer_context_t * indev_ctx,
    ...
    lv_point_t * display_res)
{
    ...
    lv_indev_add_event_cb(indev, lv_uefi_simple_pointer_indev_event_cb, LV_EVENT_DELETE, indev);
    ...
    EFI_HANDLE handle)
{
    ...
    interface = (EFI_SIMPLE_POINTER_PROTOCOL *)lv_uefi_open_protocol(handle, &_uefi_guid_simple_pointer);
    if(!lv_uefi_simple_pointer_interface_is_valid(interface)) {
        lv_uefi_close_protocol(handle, &_uefi_guid_simple_pointer);
...
static void lv_uefi_simple_pointer_indev_event_cb(lv_event_t * e)
{
    ...
    if(indev_ctx != NULL) lv_uefi_simple_pointer_context_free(indev_ctx);
}
...
static void lv_uefi_simple_pointer_read_cb(lv_indev_t * indev, lv_indev_data_t * data)
{
    ...
    for(node = lv_ll_get_head(&indev_ctx->handles); node != NULL; node = lv_ll_get_next(&indev_ctx->handles, node)) {
        ...
        lv_uefi_simple_pointer_read(indev_ctx, handle_ctx, &was_pressed);
...
static void lv_uefi_simple_pointer_context_free(lv_uefi_simple_pointer_context_t * indev_ctx)
{
    ...
    lv_ll_clear_custom(&indev_ctx->handles, lv_uefi_simple_pointer_handle_context_free);
    lv_free(indev_ctx);
}
...
static void lv_uefi_simple_pointer_handle_context_free(void * ptr)
{
    ...
    if(handle_ctx->interface) lv_uefi_close_protocol(handle_ctx->handle, &_uefi_guid_simple_pointer);
    lv_free(handle_ctx);
}
